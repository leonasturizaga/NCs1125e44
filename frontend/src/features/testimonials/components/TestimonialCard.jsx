...
//                            : "https://via.placeholder.com/800x450/333/fff?text=Video"
//                      }
//                      alt="Video thumbnail"
...
//             <div
//                className={`p-6 ${
//                   hasImage || (!hasImage && hasVideo && youtubeId) ? "pt-4" : ""
//                }`}>
...
export default function TestimonialCard({
  ...
  // EXTRAER ID DE YOUTUBE (100% fiable)
  const getYoutubeId = (url) => {
    if (!url) return null;
    try {
      const parsed = new URL(url);
  ...
  const youtubeId = getYoutubeId(testimonial.youtubeUrl);
  const isValidVideo = youtubeId && youtubeId.length === 11;
      ...
      <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-xl overflow-hidden group">
        ...
        {hasImage && (
          ...
          >
            ...
            {/* Ícono Play solo si el video es válido */}
            {hasVideo && isValidVideo && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setShowVideo(true);
        ...
        {/* Sin imagen pero con video → SOLO renderizamos la img SI el ID es válido */}
        {!hasImage && hasVideo && (
          <div className="relative h-40 bg-gray-900 overflow-hidden">
            {isValidVideo ? (
              // SOLO aquí se carga la imagen → nunca se ejecuta si ID inválido
              <img
        ...
        {/* Contenido principal */}
        <div className={`p-6 ${hasImage || (hasVideo && isValidVideo) ? "pt-4" : ""}`}>
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1 pr-4">
              <h3 className="text-lg font-semibold text-white line-clamp-2 leading-tight">
                {testimonial.title}
