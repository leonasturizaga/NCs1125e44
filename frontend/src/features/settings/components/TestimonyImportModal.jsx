...
//         data.append("description", t.description || "");
//         data.append("userId", t.userid || "");
        
//         if (t.youtubeurl) data.append("youtubeUrl", t.youtubeurl);
// if (t.categories) {
//   const cats = t.categories.split(/[,;]/).map(c => c.trim()).filter(Boolean);
//   const finalCats = cats.length >= 2 ? cats : [...cats, cats[0] || "Clients"];
//   finalCats.forEach(cat => data.append("categories", cat));
// }
//         if (t.images) {
//           // Soporta múltiples URLs separadas por coma o punto y coma
//           const urls = t.images.split(/[,;]/).map(u => u.trim()).filter(Boolean);
//           urls.forEach(url => data.append("images", url)); // backend acepta URLs como string
//         }
...
//         }
//       } catch (err) {
//         console.error("Error importando testimonio:", t.title, err);
//         errors++;
//       }
//     }
...
//     if (errors === 0) {
//       toast.success(`Importación exitosa: ${created} testimonios creados`);
//     } else {
...
//               <p>title, description, userId, categories, images (URLs separadas por , o ;), youtubeUrl (opcional)</p>
//             </div>
//           )}
...
//                 <span>{results.created} testimonios creados</span>
//               </div>
//               {results.errors > 0 && (
